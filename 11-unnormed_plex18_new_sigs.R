...
plex9 <- read_csv("unnormed_phos_sigs_new//phospho_sigs_pilot_pca1.csv")
...
group_info = read_csv("shared not normalized.csv")

p18_aug = merge(p18,group_info,by="ID")
...
g1 = ggplot(p18_sel,aes(x= enrichmentScore,y=y)) +   
  ...
  theme_classic() +
  ggtitle("Unnormed phosphosignatures enriched in Set1")+
...
g2+g1
ggsave("11-24-plex18_sigs_unnormed.pdf",width = 12, height = 8)
