...
# 读取表达矩阵数据
countMatrix <- pro_ltl[,c(17,18,19,20,25)]

# 创建一个样本信息表格
sampleTable <- data.frame(
  condition = c("Weeks1", "Weeks0", "Weeks1","Weeks0","Weeks0"),
...
fc_pro_ltl = data.frame(site = pro_ltl$id,
                        ...
                        tab)
#write.csv(fc_pro_ltl,"fc_pro_midvs0.csv",row.names = FALSE)
...
# 读取表达矩阵数据
countMatrix <- rna_ltl[,c(2,3,4,5)]

# 创建一个样本信息表格
sampleTable <- data.frame(
  condition = c("0 Weeks","0 Weeks","Mid Weeks", "Mid Weeks"),
  replicate = c(1, 2,  1, 2)
)
...
# 根据条件比较获取差异表达基因列表
results <- results(dds, contrast = c("condition",  "Mid Weeks","0 Weeks"))
...
#write.csv(fc_rna_ltl,"midvs0_mRNA.csv",row.names = FALSE)
#write.csv(fc_pro_ltl,"midvs0_pro.csv",row.names = FALSE)
...
highlight_df_sel = highlight_df %>%
  filter(group=="TF" | (fc_pro < -2)  )

g_ltl <- ggplot(cor_ltl_merge, aes(x=fc_rna, y=fc_pro)) + geom_point(alpha=0.1) + 
  ...
  scale_color_manual(values = c("Down Both" = "#193e8f", 
                                "AR Score" = "orange",
                                "NE Score" ="#7c739f",
                                "TF" = "#4c8045"))+
  ...
  xlab("mRNA")+
  ylab("Protein") +
  ggtitle("Fold change between 8/12w vs 0w")+
...
print(g_ltl)
ggsave("fc_mRNA_pro_ltl_Midvs0.pdf",height = 10,width=10,plot = g_ltl)
