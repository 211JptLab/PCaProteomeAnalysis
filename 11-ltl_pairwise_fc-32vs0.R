...
# 读取表达矩阵数据
countMatrix <- pro_ltl[,c(18,20,21,24,25,26)]

# 创建一个样本信息表格
sampleTable <- data.frame(
  condition = c("Weeks0", "Weeks0", "Weeks32", "Weeks32", "Weeks0", "Weeks32"),
  replicate = c(1, 2, 1, 2, 3, 3)
)
...
fc_pro_ltl = data.frame(site = pro_ltl$id,
                       ...
                       tab)
#write.csv(fc_pro_ltl,"fc_pro_32vs0.csv",row.names = FALSE)
...
# 读取表达矩阵数据
countMatrix <- rna_ltl[,c(2,3,6,7)]

# 创建一个样本信息表格
sampleTable <- data.frame(
  condition = c("0 Weeks", "0 Weeks","32 Weeks","32 Weeks"),
  replicate = c(1, 2,  1, 2)
)
...
# 根据条件比较获取差异表达基因列表
results <- results(dds, contrast = c("condition", "32 Weeks", "0 Weeks"))
...
#write.csv(fc_rna_ltl,"32vs0_mRNA.csv",row.names = FALSE)
#write.csv(fc_pro_ltl,"32vs0_pro.csv",row.names = FALSE)
...
#cbPalette <- c("#55b7e6", "#193e8f",  "orange","#7c739f","#4c8045")
#cbPalette <- c("#55b7e6", "#193e8f", "#4c8045")

highlight_df_sel = highlight_df %>%
  filter(group=="TF" | (fc_rna >5 & fc_pro >1) | (fc_rna < -1 & fc_pro < -1) )
...
g_ltl <- ggplot(cor_ltl_merge, aes(x=fc_rna, y=fc_pro)) + 
  ...
  ggtitle("Fold change between 32w vs 0w")+
...
ggsave("fc_mRNA_pro_ltl_32vs0_new.pdf",height = 10,width=10,plot = g_ltl)
